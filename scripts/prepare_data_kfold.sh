...
do
    ...
    for ((k=0;k<${K};k++));
    do
        python -m data.dataset --dataset ${PROCESS_DIR}/fold_${k}/test.json
        python -m data.dataset --dataset ${PROCESS_DIR}/fold_${k}/valid.json
        python -m data.dataset --dataset ${PROCESS_DIR}/fold_${k}/train.json
    done
done
